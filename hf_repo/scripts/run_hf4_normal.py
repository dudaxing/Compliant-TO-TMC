...
def run(spec_path, gamma_index, mesh_index, output):
    ...
    m = p.model
    metadata = dict(schema_version="hf4-normal-evidence-1.0", created_utc=timestamp(),
    ...
    def persist(record):
        ...
        scalar["gap_mm"] = task["geometry"]["gap_mm"] + float(p.gap_vector @ record["u"])
        scalar["index"] = i
    ...
    result = solve_prescribed_path(m, p.base, p.direction, task["targets_mm"],
...
if __name__ == "__main__":
    ...
    a = parser.parse_args()
    r = run(a.spec, a.gamma_index, a.mesh_index, a.output)
