...
def differences(left, right, label):
    """Preserve raw forces; the frozen A0 branch has zero force through d=0.25.

    Its represented-state residual force is not a physical denominator. This
    display correction changes neither force data nor any scientific gate.
    """
    ...
    for x in sorted(a.keys() & b.keys()):
        ...
        nominal_zero = right["kind"] == "A0" and right["mode"] == "uniform" and x <= .25
        rows.append(dict(coordinate=x, left_state_id=a[x]["state_id"], right_state_id=b[x]["state_id"],
                         ...
                         relative_to_abs_right=None if nominal_zero or fb == 0 else (fa-fb)/abs(fb),
                         relative_difference_reason="nominal_zero_force_reference" if nominal_zero
                         else "exact_zero_denominator" if fb == 0 else None))
...
def line(ax, run, field, *, component=None, label=None, positive=False, prefix_only=True, connect=True):
    for j, stage in enumerate(run["stages"]):
        ...
        ax.plot(xx, yy, linestyle=STYLES[run["h_mm"]] if connect else "None",
                marker="s" if not connect and run["h_mm"] == .25 else "o", ms=4 if not connect else 3,
                markerfacecolor="none" if not connect and run["h_mm"] == .25 else COLORS[run["kind"]], color=COLORS[run["kind"]],
...
def make_plots(runs, displays, protocol, output, partial):
    ...
    for run in runs:
        ...
        if run["mode"] == "uniform":
            ...
            line(axes[1], run, "normal_force_N", label=label, connect=False)
        else:
    ...
    axes[1].set(title="Before / at closure: parasites | markers = saved states", xlabel="Mean solid-bottom drive d (mm)", ylabel="Force on rigid plane (N)",
    ...
    fig.suptitle(prefix + "C1 matched comparisons | HP80 measurements | audited prefixes")
