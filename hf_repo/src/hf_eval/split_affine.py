"""Explicit affine geometric lift stages with immutable split-state handoff.

The controller below retains the frozen split prescribed controller's Newton,
Armijo, general sparse-LU, bisection, rollback and time-budget policies. Only
stage initialization and lift construction are extended. Old code is untouched.
"""
...
LIFT_SCHEME = "affine_geometric_origin_v1"
...
def solve_split_affine_path(model: TMCModel, base, direction, lift_origin, lift_shape, targets, *, initial_state=None, reaction_groups=None,
                          settings=None, force_scale_per_length, on_accept: Callable | None = None):
    """Equilibrate an explicit affine-lift stage from parameter zero.

    Stored lift is binary64 ``lift_origin + s*lift_shape``. Mechanics acts on
    the exact sum of that stored array and the stored fluctuation, not on an
    ideal three-term sum or ``u_display``. Fixed values follow base+s*direction;
    base and direction are zero on free DOFs, while origin may be nonzero there.
    The origin equals base on fixed DOFs and shape equals direction there.

    A warm start must already use this exact lift origin, bit for bit, with
    zero fixed fluctuation. No displacement is merged, rebased or normalized.
    Without one, the fluctuation starts at zero. The explicit first target 0
    always undergoes the ordinary Newton/constraint gates and is saved on
    acceptance. Only free fluctuations are corrected. Each failed increment
    preserves the previous accepted two-array state; accepted bisections stay
    in order. The parameter scale, Armijo merit, sparse LU, bounded backtracking
    and time budget are the original prescribed-controller policies.
    """
    ...
    lift_origin = _vector(lift_origin, model.ndof, "lift_origin")
    lift_shape = _vector(lift_shape, model.ndof, "lift_shape")
    if not np.array_equal(lift_origin[model.fixed_dofs], base[model.fixed_dofs]):
        raise TMCError("lift origin must match prescribed base on fixed DOFs", code="constraint_conflict")
    ...
    if levels[0] != 0:
        raise TMCError("every affine stage requires first target zero", code="invalid_input")
    if initial_state is not None:
        if not isinstance(initial_state, SplitDisplacement) or initial_state.ndof != model.ndof:
            raise TMCError("initial_state must be a full-model SplitDisplacement", code="invalid_input")
        if initial_state.lift.dtype != np.dtype("float64") or initial_state.fluctuation.dtype != np.dtype("float64"):
            raise TMCError("initial_state components must remain binary64 arrays", code="invalid_input")
        initial_lift = _vector(initial_state.lift, model.ndof, "initial_state.lift")
        initial_w = _vector(initial_state.fluctuation, model.ndof, "initial_state.fluctuation")
        if not _bitwise_equal(initial_lift, lift_origin):
            raise TMCError("initial_state lift must match lift_origin bit for bit; no implicit rebase", code="constraint_conflict")
        if np.any(initial_w[model.fixed_dofs] != 0):
            raise TMCError("initial_state fluctuation must be zero on fixed DOFs", code="constraint_conflict")
        initial = SplitDisplacement(initial_lift, initial_w)
    else:
    ...
    state_u, state_d, state_K = initial.copy(), 0.0, None
    ...
    def impose(w, d):
        try:
            ...
            if np.any(result[model.fixed_dofs] != 0):
                raise TMCError("fixed fluctuation changed", code="constraint_failure")
            with np.errstate(over="ignore", invalid="ignore"):
                # Preserve the explicit warm-start identity, including signed
                # zero bits, instead of evaluating origin + 0*shape.
                lift = lift_origin.copy() if d == 0 else lift_origin + d * lift_shape
    ...
    def advance(target, depth, original_target):
        ...
        try:
            ...
            assert (_bitwise_equal(state_u.lift, before_u.lift)
                    and _bitwise_equal(state_u.fluctuation, before_u.fluctuation)
    ...
    return {"status": "success" if failure is None else "failed", "target_reached": failure is None,
            ...
            "lift_origin": lift_origin.copy(), "lift_shape": lift_shape.copy(),
            "lift_scheme": LIFT_SCHEME, "state_sha256": state_hash(state_u),
            "initial_state_provided": initial_state is not None,
            "initial_state_sha256": state_hash(initial),
            "initial_u_lift": initial.lift.copy(), "initial_u_fluctuation": initial.fluctuation.copy(),
            ...
            "reference_state": "provided_split_state_at_zero_target" if initial_state is not None else "geometric_lift_origin_at_zero_target",
            "constraint_force_sign": "apparatus_on_model", "external_nodal_loads": "none",
            "scope": "explicit-origin split affine stage; requires independent precision and physical validation"}
