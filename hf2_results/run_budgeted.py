...
parser.add_argument('--category', choices=['small','compile','matlab_cshape','python_cshape','isolated'], required=True)
...
small_used = sum(j.get('wall_seconds', j.get('reserved_limit_seconds', 0)) for j in prior if j['category'] == 'small')
category_cap = 300 if args.category in ['small','isolated'] else 600 if args.category == 'compile' else 1200
limit = min(args.limit, category_cap, 3600 - used)
