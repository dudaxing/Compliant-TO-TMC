...
temporary=Path(tempfile.mkdtemp(prefix='hf2_detached_'))
...
with (root/'hf2_results/detached_install_offline.log').open('wb') as log:
...
command=[str(python),'-I',str(copy/'scripts/run_hf2_isolated.py'),'--repository',str(copy),
...
location={'detached_root_record_only':str(temporary),'copied_repository':str(copy),
          ...
          'dependency_install_method':'exact versions from runtime lock, offline uv cache; online require-hashes attempt retained separately after CDN timeout; installed RECORD hashes checked during replay'}
(root/'hf2_results/detached_location.json').write_text(json.dumps(location,indent=2),encoding='utf-8')
monitor=[sys.executable,str(root/'hf2_results/run_budgeted.py'),'--name','hf2_detached',
...
if (temporary/'acceptance').exists():
    shutil.copytree(temporary/'acceptance',root/'hf2_results/detached',dirs_exist_ok=True)
...
(root/'hf2_results/detached_receipt.json').write_text(json.dumps(receipt,indent=2),encoding='utf-8')
