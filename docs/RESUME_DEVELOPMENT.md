...
**当前失败触发停止条件，不继续或自动重跑 FE。** 公开数值读回通过也不解除这个停止条件。下一次新路径前必须先完成已接受的 [P1 完整启动合同修复](HF4_C2_CONSOLIDATED_REVIEW_20260921.md)：wrapper 和直接 runner 均在创建输出及派发前校验完整合同与获准协议身份；该修复尚未实现，不得原地修改冻结 v3。同时须完成[稳定可微运动学修复计划](HF4_C2_KINEMATICS_REPAIR_PLAN.md)规定的无求解精度和一致切线核验，再另行冻结新协议、源码与复验计划。旧 wrapper 及复制 v3 的接口只表示历史，不能用来绕过这两项前置。任何新的计划仍需明确自己的预算和停止条件；任何失败、超时、来源变化或不完整目标立即停止，不删除回执重置预算，不自动重试。历史预算为每条求解外部上限 700 秒、三条累计 2100 秒；每条独立审计最多 1200 秒、累计 3600 秒，不自动成为新计划预算。历史路径和旧失败积分观察始终保留。0.5.0 wheel 不含 C2，应使用主分支源码安装与脚本。
...
S0 完成后，先落实上述尚未实现的 **P1 完整启动合同前置校验**，保留冻结 v3 原件；随后推进稳定 F、完整内力和一致切线的有界无求解验证。两项证据齐备后才制定新路径复验，S0 本身没有实施这些力学或启动器修改。
