"""Sequential monitored HF4 jobs; categories are frozen in validation_spec.json."""
...
spec_path=root/'hf4_repair_results/execution_spec.json'
spec=json.loads(spec_path.read_text()); limits=spec['resource_limits']
jobs=root/'hf4_repair_results/resource_jobs'; jobs.mkdir(exist_ok=True)
...
r={'name':a.name,'category':a.category,'status':'running','process_started':False,'started_utc':datetime.now(timezone.utc).isoformat(),
 ...
 'command_file_sha256':{str((a.cwd/arg).resolve()):hashlib.sha256((a.cwd/arg).read_bytes()).hexdigest() for arg in command[1:] if (a.cwd/arg).is_file()},
