...
def validate_production_acceptance(arrays, record, model, spec, force_scale_per_length):
    """Recompute the production stop from saved forces, before HP acceptance."""
    ndof = len(model["base"])
    f = arrays["internal_force"]
    J = arrays["J"]
    if (f.shape != (ndof,) or f.dtype != np.dtype("float64") or not np.all(np.isfinite(f))
            or J.shape != (len(model["connectivity"]),9) or J.dtype != np.dtype("float64")
            or not np.all(np.isfinite(J)) or np.any(J <= 0)):
        raise ValueError("production force/J must be finite binary64 in the declared domain")
    fixed = model["fixed_dofs"]
    free = np.setdiff1d(np.arange(ndof),fixed)
    free_norm, fixed_norm = float(np.linalg.norm(f[free])), float(np.linalg.norm(f[fixed]))
    dscale = max(abs(record["d"]),spec["solver"]["displacement_scale_floor"])
    floor = spec["solver"]["force_scale_floor_factor"]*force_scale_per_length*dscale
    scale = max(free_norm,fixed_norm,floor)
    relative = free_norm/scale
    expected = dict(relative_residual=relative,free_force_residual_norm=free_norm,
                    internal_free_norm=free_norm,reaction_fixed_norm=fixed_norm,
                    residual_scale=scale,force_scale_floor=floor,displacement_scale=dscale)
    if not all(np.isfinite(value) for value in expected.values()) or scale <= 0:
        raise ValueError("invalid production force scale")
    if relative > spec["solver"]["tolerance"]:
        raise ValueError("saved accepted state exceeds original production stopping tolerance")
    for name,value in expected.items():
        if record[name] != value:
            raise ValueError("archived production metric does not match saved force: "+name)
    return expected


def derived_status(checks):
    numerical = "pass" if all(q["status"]=="pass" for q in checks if q["kind"]=="numerical") else "not_pass"
    models = [q for q in checks if q["kind"]=="model"]
    model = "measurement_only" if not models else "pass" if all(q["status"]=="pass" for q in models) else "not_pass"
    return numerical,model,"pass" if numerical=="pass" and model in ("pass","measurement_only") else "not_pass"
...
def audit(run, output, spec_path, source_freeze):
    ...
    paths += [Path(__file__), Path(__file__).with_name("hf4_split_precision_reference.py"),
    ...
    requested = meta["requested_targets"]
    if meta["execution_scope"] == "first_target_probe":
        if requested != spec["targets_mm"][:2]:
            raise ValueError("first-target probe must cover the original first two targets")
    elif meta["execution_scope"] != "full_path" or requested != spec["targets_mm"]:
        raise ValueError("unknown scope or changed full targets")
    if meta["state_representation"] != "split_displacement_v1" or meta["lift_scheme"] != "normal_geometric_piecewise_v1":
        raise ValueError("unsupported split-state schema or lift")
    validate_state_inventory(entries, result, requested,
    ...
    expected_shape = np.zeros(len(m["base"]))
    H1, g0 = D(spec["geometry"]["lower_height_mm"]), D(spec["geometry"]["gap_mm"])
    for i, xy in enumerate(m["coordinates"]):
        y = D(xy[1])
        expected_shape[2*i+1] = float(Decimal(1) if y <= H1 else Decimal(0) if y >= H1+g0 else (H1+g0-y)/g0)
    if m["lift_shape"].shape != expected_shape.shape or not np.array_equal(m["lift_shape"], expected_shape):
        raise ValueError("geometric lift differs from independent construction")
    rows = []
    for entry in entries:
        ...
        if state_path.exists():
            ...
            if tuple(cached[k] for k in ("numerical_status","model_status","status")) != derived_status(cached["checks"]):
                raise ValueError("cached aggregate status differs from checks")
            rows.append(cached)
            continue
        a, r = read_npz(run/"steps"/entry["file"]), read_json(run/"steps"/entry["metadata"])
        ndof = len(m["base"])
        for field in ("u_lift", "u_fluctuation", "u_display", "internal_force", "material_internal_force", "regularization_internal_force", "support_reaction"):
            if a[field].shape != (ndof,):
                raise ValueError(f"archived {field} shape differs from model DOFs")
        if r["state_representation"] != "split_displacement_v1":
            raise ValueError("state schema differs")
        digest = hashlib.sha256(b"split_displacement_v1")
        digest.update(np.asarray([ndof], dtype="<i8").tobytes())
        for key in ("u_lift", "u_fluctuation"):
            if a[key].dtype != np.dtype("float64"):
                raise ValueError("split state must store actual binary64 arrays")
            digest.update(np.asarray(a[key], dtype="<f8").tobytes())
        if digest.hexdigest() != r["state_sha256"]:
            raise ValueError("split state hash differs from actual two arrays")
        with localcontext() as lift_context:
            lift_context.prec = 100
            for i in range(ndof):
                if D(a["u_lift"][i]) != D(m["base"][i])+D(r["d"])*D(m["lift_shape"][i]):
                    raise ValueError("actual lift does not equal the frozen dyadic construction")
        if np.any(a["u_fluctuation"][m["fixed_dofs"]] != 0):
            raise ValueError("fixed fluctuation must be zero")
        if not np.array_equal(a["u_display"], a["u_lift"]+a["u_fluctuation"]):
            raise ValueError("display view differs from declared lossy projection")
        production_metrics = validate_production_acceptance(a,r,m,spec,meta["force_scale_per_length"])
        hp = evaluate_split_prescribed_state(model, a["u_lift"], a["u_fluctuation"], r["d"], m["base"], m["direction"], groups,
        ...
        with localcontext() as ctx:
            ...
            height = H1+H2+g0
            d = D(r["d"])
            check("production_free_balance", D(production_metrics["relative_residual"]), D(spec["solver"]["tolerance"]))
            ...
            for field, hpfield in (("internal_force","internal_decimal"),
                ...
                free_error = norm([D(a[field][i])-hp[hpfield][i] for i in hp["free_dofs"]])/sf
                check("free_evaluation_"+field, free_error, Decimal(c["force_evaluation_relative_budget"]))
            ...
            gap = sum((D(w)*(D(m["coordinates"][i,1])+(D(a["u_lift"][2*i+1])+D(a["u_fluctuation"][2*i+1]))
                             -D(m["coordinates"][j,1])-(D(a["u_lift"][2*j+1])+D(a["u_fluctuation"][2*j+1])))
            ...
            for i, (_, raw_y) in enumerate(m["coordinates"]):
                ...
                errors.extend([abs((D(a["u_lift"][2*i])+D(a["u_fluctuation"][2*i]))), abs((D(a["u_lift"][2*i+1])+D(a["u_fluctuation"][2*i+1]))-expected)])
            ...
            cross = None
            if r["d"] == requested[-1]:
                hp80 = evaluate_split_prescribed_state(model, a["u_lift"], a["u_fluctuation"], r["d"], m["base"], m["direction"], groups,
    ...
    if any(sha(path) != expected for path,expected in bindings.items()):
        raise ValueError("audit inputs changed during evaluation")
    result = read_json(run/"result.json")
    complete = len(rows)==len(entries) and result["status"]=="success"
    summary = dict(schema_version="hf4-split-normal-audit-1.0", created_utc=timestamp(),
                   ...
                   execution_scope=meta["execution_scope"], requested_targets=requested,
                   completed_original_targets=[r["d"] for r in result["accepted_steps"] if r["is_original_target"]],
                   target_displacement=result["target_displacement"], inputs_unchanged_at_end=True,
