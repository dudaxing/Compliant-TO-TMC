...
repo, out = root/"hf_repo", root/"hf4_results"
...
temporary = Path(tempfile.mkdtemp(prefix="hf4_detached_"))
inputs = temporary/"input"
inputs.mkdir()
shutil.copy2(wheel, temporary/wheel.name)
shutil.copy2(repo/"scripts/run_hf4_isolated.py", temporary/"run_hf4_isolated.py")
shutil.copy2(repo/"configs/hf4/validation_spec.json", inputs/"validation_spec.json")
shutil.copy2(out/"g1_m1_001/result.json", inputs/"failed_reference.json")
...
for gi in (0,1):
    ...
    np.savez_compressed(inputs/f"reference_{gi}.npz", **arrays)
...
foreign.mkdir()
command = [str(python),"-I",str(temporary/"run_hf4_isolated.py"),"--input",str(inputs),"--output",str(temporary/"acceptance"),"--forbid",str(root)]
