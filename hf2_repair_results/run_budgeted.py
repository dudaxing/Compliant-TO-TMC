...
amendment_path = ROOT/'hf2_repair_results/resource_amendment_001.json'
if amendment_path.exists():
    amendment_record = json.loads(amendment_path.read_text(encoding='utf-8'))
    assert amendment_record['original_postprocess_cumulative_seconds'] == 600
    assert amendment_record['amended_postprocess_cumulative_seconds'] == 720
    assert amendment_record['total_numerical_seconds'] == 3000
    assert amendment_record['continuation_process_seconds'] == 120
    assert hashlib.sha256((ROOT/'hf_repo/validation/hf2_repair/precision_spec.json').read_bytes()).hexdigest() == amendment_record['original_precision_spec_sha256']
    postprocess_cap = 720
    if args.category == 'postprocess':
        assert args.name == amendment_record['continuation_job_name'], 'Only the named offline continuation is covered by this amendment'
        assert not any(j['name'] == args.name and j.get('process_started', True) for j in prior), 'The bounded continuation has already been used'
        assert args.limit <= 120
category_cap = 300 if args.category in ['small','compile','isolated'] else 120 if args.category == 'postprocess' and amendment_record else 600 if args.category == 'postprocess' else 1200
limit = min(args.limit, category_cap, 3000 - used)
if args.category in ['small','compile','isolated']:
    limit = min(limit, 1200 - small_used)
if args.category == 'postprocess':
    limit = min(limit, postprocess_cap - sum(j.get('wall_seconds', j.get('reserved_limit_seconds',0)) for j in prior if j['category']=='postprocess'))
...
receipt['process_started'] = False
if amendment_record:
    receipt['resource_amendment_sha256'] = hashlib.sha256(amendment_path.read_bytes()).hexdigest()
def save():
...
with log_path.open('wb') as log:
    try:
        ...
        receipt.update(status='launch_failed', returncode=1, stop_reason='launch_error',
                       wall_seconds=time.perf_counter()-started, sampled_peak_tree_rss_bytes=0,
                       memory_samples=0, exception_type=type(exc).__name__, message=str(exc))
        save()
